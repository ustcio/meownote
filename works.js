...
async function handleChat(request, env, ctx) {
  ...
  const apiKey = config.provider === 'qwen' ? env.MEOWNOTE_ALIYUN_KEY : env.DOUBAO_API_KEY;
...
async function getWorkspaceFile(env, fileId) {
  try {
    const result = await env.DB.prepare(
    ...
    return jsonResponse({ success: false, message: 'Failed to get file', error: error.message }, 500);
  }
}
...
async function updateWorkspaceFile(request, env, fileId) {
  try {
    ...
    return jsonResponse({ success: false, message: 'Failed to update file', error: error.message }, 500);
  }
}

async function deleteWorkspaceFile(env, fileId) {
  try {
    const existing = await env.DB.prepare(
      'SELECT * FROM workspace_files WHERE id = ?'
    ).bind(fileId).first();
    ...
    console.log('[Workspace] Deleted file:', fileId);
